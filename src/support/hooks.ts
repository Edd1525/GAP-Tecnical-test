...
/**
 * Create report metadata json with all information to show on final report
 */
function createReportMetadata(metadata) {
  const metadataJSON = JSON.stringify(metadata);
  fs.writeFile("data/metadata_report.json", metadataJSON, (err) => {
    if(err) {
      console.log(err);
    }
  });
...
After(async function(testCase) {
  ...
  const metadata = {
    'App Version': '0.0.1', // For the future try to get command center version with endpont according to the url
    'Browser': t.browser.name + ' ' + t.browser.version,
    'Executed': t.browser.os.name + ' ' + t.browser.os.version,
    'Parallel': 'Scenarios',
    'Platform': t.browser.platform,
    'Test Environment': 'https://vacations-management.herokuapp.com/employees'
  };

  createReportMetadata(metadata);
...
const generateHtmlReport = () => {
  if (GENERATE_CUCUMBER_HTML_REPORT) {
    try {
      child_process.exec(`node cucumber-html.config.js`);
    } catch (error) {
...
const generateJunitReport = () => {
  if (GENERATE_CUCUMBER_JUNIT_REPORT) {
    try {
      child_process.exec(`node cucumber-junit.config.js`);
    } catch (error) {
